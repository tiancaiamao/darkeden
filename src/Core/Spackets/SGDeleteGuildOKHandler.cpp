		...
		BEGIN_DB
		{
			...
			for (; itr != Members.end(); itr++ )
			{
				...
				if (pCreature != NULL && pCreature->isPC() )
				{
					...
					{
                        Gold = min(Gold + (unsigned long)RETURN_SLAYER_MASTER_GOLD, (unsigned long)2000000000);
					}
					else if (pGuildMember->getRank() == GuildMember::GUILDMEMBER_RANK_SUBMASTER )
					{
                        Gold = min(Gold + (unsigned long)RETURN_SLAYER_SUBMASTER_GOLD, (long unsigned)2000000000);
					}
